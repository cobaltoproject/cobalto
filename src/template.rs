...
pub fn tokenize_template(content: &str) -> Vec<Token> {
    ...
    for mat in re.find_iter(content) {
        ...
        if m.starts_with("{{") {
            let inner = m.trim_start_matches("{{").trim_end_matches("}}").trim().to_string();
...
fn parse_nodes(tokens: &[Token], idx: &mut usize, end_tags: &[&str]) -> Vec<Node> {
    ...
    while *idx < tokens.len() {
        match &tokens[*idx] {
            Token::Text(t) => { nodes.push(Node::Text(t.clone())); *idx += 1; }
            Token::Variable(v) => { nodes.push(Node::Variable(v.clone())); *idx += 1; }
            Token::Tag(tag) => {
                let t = tag.trim();
                if end_tags.contains(&t) { break; }
                // Handle extends
                ...
                if let Some(name) = t.strip_prefix("block ") {
                    ...
                    *idx += 1; // skip endblock
                    nodes.push(Node::Block { name: name.to_string(), body });
                    continue;
                }
                ...
                if let Some(cond) = t.strip_prefix("if ") {
                    ...
                    *idx += 1; // skip endif
                    nodes.push(Node::If { condition: cond.to_string(), then_body, else_body });
                    continue;
                }
                // Handle for/endfor
    ...
    nodes.iter().map(|node| match node {
        Node::Block { name, body } => {
            if let Some(child) = child_blocks.get(name) {
                Node::Block { name: name.clone(), body: child.clone() }
            } else {
                Node::Block { name: name.clone(), body: merge_blocks(body, child_blocks) }
            }
        }
        ...
        Node::For { var_name, list_name, body } => Node::For {
...
fn render_nodes(nodes: &[Node], context: &HashMap<String, TemplateValue>) -> String {
...
pub fn render_template(template_name: &str, context: &HashMap<String, TemplateValue>) -> Response {
    // Load child template
    let child = std::fs::read_to_string(format!("templates/{}", template_name)).unwrap();
